...
def get_image_size(filename: str):
    ...
    im = cv2.imread('../start_frame/' + file_ + '_start_frame.png')
    return im.shape[:2]
...
def crop_video(filename: str, x_pos: float = 0, y_pos: float = 200, width: float = 650, height: float = 900) -> None:
    ...
    '''
    input_video = ffmpeg.input(f"{vid_path + filename}.mkv")
    cropped_video = ffmpeg.crop(input_video, x_pos, y_pos, width, height)
    output_video = ffmpeg.output(cropped_video, f"{'../vid_cropped/' + filename}_cropped.mkv")
    ffmpeg.run(output_video)
    
def get_audio(filename: str):
    cmd_get_audio = f" ffmpeg -i {vid_path + filename}.mkv -q:a 0 -map a {'../audio/' + filename}.mp3 -y"
